...
parser = argparse.ArgumentParser(description='DeepRacer Controller - Robust Version (Python 2.7)')
...
def main():
    """Main function"""
    print("\n=== DeepRacer Controller - Robust Version (Python 2.7) ===")
    ...
    try:
        ...
        try:
            ...
            if "is_control_ready" in str(e):
                ...
                print("Current theta value: {0:.6f}".format(float(state_str.split(',')[3])))
                print("Please reorient the DeepRacer to have a theta value closer to 0")
                print("Acceptable theta range: [{0}, {1}]".format(THETA_MIN, THETA_MAX))
                return
            else:
                raise
        except Exception as e:
