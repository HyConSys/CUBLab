...
def get_optitrack_data(first_connection=False):
    ...
    if not first_connection and optitrack_cache is not None:
        if current_time - last_optitrack_time < CACHE_VALIDITY_PERIOD:
            print(f"Using cached OptiTrack data ({current_time - last_optitrack_time:.1f} seconds old)")
    ...
    try:
        ...
        print(f"Connecting to OptiTrack with {timeout} second timeout...")
        start_time = time.time()
        
        # Use requests library instead of httplib2
        session = requests.Session()
        session.trust_env = False  # Don't use environment proxies
        
        response = session.get(LOCALIZATION_SERVER_URL, timeout=timeout)
        elapsed = time.time() - start_time
        print(f"OptiTrack connection established in {elapsed:.2f} seconds")
        ...
        print(f"Connection to OptiTrack timed out after {timeout} seconds")
        return None
...
def get_next_action(actions_list, state, logger=None):
    ...
    try:
        ...
        normalized_state = f"{x},{y},{theta},{v}"
...
def main():
    """Main function"""
    print("\n=== DeepRacer Hybrid Controller (Python 3.8) ===")
    ...
    print(f"pFaces server URL: {PFACES_SERVER_URL}")
    print(f"OptiTrack server URL: {LOCALIZATION_SERVER_URL}")
    print(f"Cache validity period: {CACHE_VALIDITY_PERIOD} seconds")
